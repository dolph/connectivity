...
func PrintUsage() {
	...
	fmt.Println("")
	fmt.Println("Usage: connectivity [-v|--verbose] <command>")
	fmt.Println("")
...
func PrintCommandUsage(command string) bool {
	if command == "check" {
		...
		fmt.Println("")
		fmt.Println("Usage: connectivity [-v|--verbose] check [urls]")
		fmt.Println("")
		fmt.Println("With -v, logs each resolved IP and selected route during checks.")
		fmt.Println("")
		fmt.Println("Exit codes:")
		fmt.Println("  0  All destinations reachable")
		fmt.Println("  1  At least one destination unreachable, no config file found, or no destinations parsed")
		fmt.Println("  2  Destination URL parse failure or invalid subcommand")
		fmt.Println("")
		...
		fmt.Println("")
		fmt.Println("On startup, logs version, config path, statsd endpoint, and destination count.")
		fmt.Println("")
		...
		fmt.Println("")
		fmt.Println("Exit codes: 0 on success, 1 on validation errors. Only the config")
